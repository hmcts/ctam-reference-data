...
Sync Impact Report
==================
Version change: 1.0.0 → 1.1.0 (MINOR: guidance materially expanded. Principle VII and two
constraints are redefined, which would normally be MAJOR, but v1.0.0 was never committed
to main or applied to code, so the team agreed to treat this as MINOR.)

Modified principles:
- III. Security Baked-In, Not Bolted On → unchanged title; adds local-only default
  credential exemption, OWASP suppression rules and the Renovate/OWASP mechanism.
- IV. Transparency - Work in the Open → unchanged title; now sanctions the shared
  HMCTS repositories (cnp-flux-config, cnp-jenkins-config, shared-infrastructure)
  as locations for deployment and infrastructure configuration.
- V. Peer Review - Nothing Reaches Production Without Review → unchanged title;
  adds an exception for Renovate patch/minor dependency updates.
- VII. Domain Driven Design - Design Around the Domain → unchanged title; REDEFINED:
  package layout is now technical sub-packages under the root
  uk.gov.hmcts.ctam.refdata (HMCTS template layout). DDD now governs naming, model
  separation and bounded contexts rather than folder structure. This reverses the
  v1.0.0 rule "not around technical layers".

Added sections:
- Additional Constraints → Runtime model
- Additional Constraints → Naming and ownership
- Additional Constraints → Infrastructure and deployment
- Additional Constraints → Approved baseline libraries
- Development Workflow → quality gates (coverage threshold, test source sets,
  PR template Constitution Check, living documents under specs/)

Removed sections: none

Redefined constraints (backward-incompatible wording, flagged for reviewers):
- Technology stack: Liquibase → Flyway for schema migrations.
- Principle IV: "all pipeline definitions MUST live in this repository" → now
  permits the shared HMCTS configuration repositories.

Follow-up TODOs:
- Observability and CI tooling specifics (logging library, correlation IDs,
  Application Insights, CodeQL, nightly pipeline) deliberately left generic until
  aligned with HMCTS shared tooling. See Additional Constraints → Observability and CI.

Templates checked: plan-template.md, spec-template.md, tasks-template.md read the
constitution at runtime; no edits required.
-->
